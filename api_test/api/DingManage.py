...
from api_test.config.Glb_config import APPID
...
class DingManage(APIView):
    ...
    @extend_schema(
        summary="获取钉钉配置",
        description="获取钉钉应用 App ID",
        responses={
            200: success_response("成功", data_example={"app_id":"dingoap..."}),
        },
    )
    ...
    @extend_schema(
        summary="钉钉登录",
        description="使用钉钉授权码登录系统",
        request=json_body(
            properties={
                "code": {"type": "string", "description": "钉钉授权临时 code"},
            },
            required=["code"],
        ),
        responses={
            200: success_response("成功", data_example={"key":"token_string","user":1,"userphoto":"/file/userphoto.jpg"}),
            **error_responses(),
        },
    )
