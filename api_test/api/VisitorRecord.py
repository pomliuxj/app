...
class Record(APIView):
    ...
    @extend_schema(
        summary="记录访客信息",
        description="记录访客的地理位置信息",
        request=json_body(
            properties={
                "success": {"type": "integer", "description": "是否成功获取位置 (0:失败, 1:成功)", "enum": [0, 1]},
                "longitude": {"type": "number", "description": "经度（success=1时必填）"},
                "latitude": {"type": "number", "description": "纬度（success=1时必填）"},
            },
            required=["success"],
        ),
        responses={
