...
class ApiHead(models.Model):
    ...
    value = models.CharField(max_length=2048, blank=True, null=True, verbose_name='内容')
...
class ApiResponse(models.Model):
    ...
    name = models.CharField(max_length=1024, verbose_name="参数名")
    tier = models.CharField(max_length=2048, blank=True, default="", verbose_name="层级路径")
    _type = models.CharField(default="String", max_length=1024, verbose_name='参数类型',
                             choices=(('Int', 'Int'), ('String', 'String')))
    value = models.CharField(max_length=1024, blank=True, null=True, verbose_name='参数值')
...
class AutomationHead(models.Model):
    ...
    name = models.CharField(max_length=1024, verbose_name='参数名')
    value = models.CharField(max_length=2048, verbose_name='内容')
...
class AutomationTestTask(models.Model):
    ...
    caseId = models.CharField(max_length=50, verbose_name='选中case', null=True)
...
class AutomationTaskRunTime(models.Model):
    ...
    taskName = models.CharField(max_length=50, verbose_name='任务名称', null=True)
    taskResult = models.BooleanField(default=True, verbose_name='任务执行结果')
    caseRunDetail = models.CharField(max_length=1024, verbose_name='用例执行明细', null=True)
...
class AutomationCaseTestResult(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=['automationCaseApi', 'testTime']),
        ]
...
class OnlineCode(models.Model):
    """
    全局变量
    """
    id = models.AutoField(primary_key=True)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, verbose_name='项目', default=None)
    Code = models.TextField(null=True, verbose_name='在线代码')
    variablesName = models.CharField(max_length=50, null=True, verbose_name='变量名称')

    class Meta:
        verbose_name = "全局变量"
        verbose_name_plural = "全局在线代码"


class AutomationJsonCheck(models.Model):
    """
    JSON参数校验规则
    """
    id = models.AutoField(primary_key=True)
    automationCaseApi = models.ForeignKey(AutomationCaseApi, related_name='jsonCheckDetail',
                                          on_delete=models.CASCADE, verbose_name='接口id')
    name = models.CharField(max_length=50, verbose_name='JSON名称', blank=True, null=True)
    checkType = models.CharField(max_length=50, verbose_name='校验方式', choices=CHECK_TYPE)
    checkRule = models.CharField(max_length=512, verbose_name='JSON名称', blank=True, null=True)
    value = models.TextField(verbose_name='检验值', blank=True, null=True)
...
class DataBaseInfo(models.Model):
    ...
    """
    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=50, verbose_name='数据源名称', blank=True, null=True)
    user = models.CharField(max_length=50, verbose_name='数据库用户', blank=True, null=True)
    password = models.CharField(max_length=50, verbose_name='数据库用户', blank=True, null=True)
    host = models.CharField(max_length=50, verbose_name='数据库ip或域名', blank=True, null=True)
    port = models.IntegerField(verbose_name='数据库端口', blank=True, null=True)
    db = models.CharField(max_length=50, verbose_name='要选择的数据库', blank=True, null=True)
...
class CaseDataExcute(models.Model):
    """
    校验数据库查询结果
    """
    id = models.AutoField(primary_key=True)
    name = models.CharField(max_length=50, verbose_name='字段名称', unique=True, blank=True, default=None)
    AutomationCaseApi = models.ForeignKey(AutomationCaseApi, related_name='CaseDataExcutedatail',
                                          on_delete=models.CASCADE, verbose_name='接口id', default=None)
    dataInfo = models.ForeignKey(DataBaseInfo, related_name='databaseinfodetail',
                                 on_delete=models.CASCADE, verbose_name='数据库配置详情')
    excutesql = models.TextField(verbose_name='数据执行sql', default=None)
    pre_excute = models.BooleanField(verbose_name='是否前置执行', default=True)
    type = models.CharField(max_length=10, verbose_name='数据类型', choices=DATA_TYPE)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, verbose_name='项目', default=None)
    ...
    class Meta:
        verbose_name = '校验数据库查询结果'
        verbose_name_plural = '校验数据库查询结果'
